...
var gameConfig = {
  ...
  imageurl:
    "https://images.unsplash.com/photo-1634654422250-9f4526b8453c?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=411&q=80", //Path to picture
...
function Block(row, cols) {
  ...
  this.div.style.background = `url("${gameConfig.imageurl}") -${
    this.correctCols * this.width
  }px -${this.correctRow * this.height}px`;
  this.div.style.border = "1px solid #fff";
  this.div.style["box-sizing"] = "border-box";
  this.div.style.position = "absolute";

  this.show = function () {
    //Show the location of the small box
    this.div.style.left = this.cols * gameConfig.pieceWidth + "px";
    this.div.style.top = this.row * gameConfig.pieceHeight + "px";
  };
  this.show();

  if (row === gameConfig.row - 1 && cols === gameConfig.cols - 1) {
    //Last box hidden
  ...
  this.isCorrect = function () {
    //Determine whether the row and column of the current position of the block is equal to the row and column of the correct position
...
function init() {
  ...
  function randomSort() {
    ...
    for (var i = 0; i < step; i++) {
      ...
      for (var j = 0; j < blocks.length; j++) {
        ...
        if (
          (blocks[j].row === blankBlock[0].row &&
            Math.abs(blocks[j].cols - blankBlock[0].cols) === 1) ||
          (blocks[j].cols === blankBlock[0].cols &&
            Math.abs(blocks[j].row - blankBlock[0].row) === 1)
        ) {
    ...
    blocks.forEach(function (item) {
      item.div.onclick = function () {
        if (gameConfig.isOver) {
          //At the end of the game, the following operations will not be continued
          return;
        }
        //Judge whether it can be exchanged. If the abscissa is the same, the ordinate is 1||If the ordinates are the same, the abscissa difference is 1
        if (
          (item.row === isDisplayBlock.row &&
            Math.abs(item.cols - isDisplayBlock.cols) === 1) ||
          (item.cols === isDisplayBlock.cols &&
            Math.abs(item.row - isDisplayBlock.row) === 1)
        ) {
  ...
  //game over
  function isWin() {
    var wrongBlocks = blocks.filter(function (item) {
      //Filter out the wrong box
      return !item.isCorrect();
    });
    if (wrongBlocks.length === 0) {
      //All the squares are in the right place
